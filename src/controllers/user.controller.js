...
const loginUser= asyncHandler(async(req, res)=> {
  ...
  if (!username && !email) { // (!(username \\ email)) can be alternative 
  ...
  const options = {
    httpOnly: true ,  // can be access from backend only , frontend only read me mode
    secure: true
  }
...
const getCurrentUser = asyncHandler(async(req, res)=> {
  return res
  .status(200)
  .json(200, req.res, "current user Fetched")
})
